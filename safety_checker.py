...
class StableDiffusionSafetyChecker(PreTrainedModel):
    ...
    def __init__(self, config: CLIPConfig):
        ...
        self.concept_embeds = nn.Parameter(torch.ones(
            17, config.projection_dim), requires_grad=False)
        self.special_care_embeds = nn.Parameter(torch.ones(
            3, config.projection_dim), requires_grad=False)
    ...
    def forward(self, clip_input, images):
        ...
        special_cos_dist = cosine_distance(
            image_embeds, self.special_care_embeds).cpu().numpy()
        cos_dist = cosine_distance(
            image_embeds, self.concept_embeds).cpu().numpy()
